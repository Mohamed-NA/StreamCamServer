...
from streamcamserver.paths import CONFIG_DIR, PROJECT_ROOT
...
LABEL_COLORS = {
    ...
    "mask_weared_incorrectly": (0, 165, 255),
    "mask_weared_incorrect": (0, 165, 255),
}
_DEFAULT_COLOR = (180, 180, 180)


def _prep_mobilenet(img_bgr: np.ndarray, size: tuple[int, int]) -> np.ndarray:
...
def _prep_imagenet_nchw(img_bgr: np.ndarray, size: tuple[int, int]) -> np.ndarray:
    img = cv2.resize(img_bgr, size)
...
def _prep_rcnn_nchw(img_bgr: np.ndarray, size: tuple[int, int]) -> np.ndarray:
    img = cv2.resize(img_bgr, size)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB).astype("float32") / 255.0
    img = img.transpose(2, 0, 1)
    return img[np.newaxis]
...
class _OnnxDetector(_BaseModel):
    ...
    def predict(self, frame: np.ndarray) -> list[dict[str, Any]]:
        ...
        boxes, labels, scores = self._session.run(None, {self._input_name: inp})
...
def _make_model(cfg: dict[str, Any]) -> _BaseModel:
    backend = cfg["backend"]
    model_type = cfg["type"]
    if backend == "tflite" and model_type == "classifier":
        return _TFLiteClassifier(cfg)
    if backend == "onnx" and model_type == "classifier":
        return _OnnxClassifier(cfg)
    if backend == "onnx" and model_type == "detector":
        return _OnnxDetector(cfg)
    raise ValueError(f"Unsupported backend/type combination: {backend}/{model_type}")
...
class ModelManager:
    def __init__(self, config_path: str | Path = CONFIG_DIR / "models.json"):
        self._config_path = _resolve_path(config_path, base_dir=PROJECT_ROOT)
        ...
        for model_cfg in config["models"]:
            ...
            cfg_copy["_resolved_path"] = _resolve_path(model_cfg["path"], base_dir=config_dir)
        ...
        default = config.get("default", config["models"][0]["id"])
        if self._configs[default]["_resolved_path"].exists():
            self.switch(default)
        else:
            for model_id, cfg in self._configs.items():
                if cfg["_resolved_path"].exists():
                    self.switch(model_id)
                    break
        ...
        return [
            {
                ...
                "available": model["_resolved_path"].exists(),
                "active": model["id"] == self._active,
            }
    ...
    def switch(self, model_id: str) -> dict[str, Any]:
        ...
        if not model_path.exists():
            raise FileNotFoundError(
                f"Model file not found: {model_path}. Run the training/export flow first."
            )
    ...
    def predict(self, frame: np.ndarray) -> list[dict[str, Any]]:
        ...
        return [] if model is None else model.predict(frame)

    def draw(self, frame: np.ndarray, results: list[dict[str, Any]]) -> np.ndarray:
        for result in results:
            ...
            cv2.rectangle(frame, (x, y - text_h - 12), (x + text_w + 8, y), color, -1)
            cv2.putText(
                frame,
                text,
                (x + 4, y - 5),
