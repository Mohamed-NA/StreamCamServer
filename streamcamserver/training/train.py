...
from streamcamserver.paths import CACHE_DIR, CHECKPOINT_DIR, DATA_DIR, NOTEBOOK_DIR
from streamcamserver.training.data import FaceMaskDataset, collate_fn, require_child_text
...
def train_vit(args: argparse.Namespace) -> None:
    ...
    split = Dataset.from_pandas(hf_df, preserve_index=False).train_test_split(test_size=0.2, seed=42)
    ...
    def preprocess_batch(batch: dict[str, list[Any]]) -> dict[str, list[Any]]:
        ...
        return {"pixel_values": [arr for arr in pixel_values], "labels": batch["label"]}
    ...
    training_args = TrainingArguments(
        output_dir=str(NOTEBOOK_DIR / "vit-face-mask"),
    ...
    trainer.train()
    print("ViT evaluation:", trainer.evaluate())
...
def split_detection_dataset(
    ...
    return (
        all_images[:n_train],
        all_images[n_train:n_train + n_val],
        all_images[n_train + n_val:],
    )
...
def train_rcnn(args: argparse.Namespace) -> None:
    ...
    for epoch in range(1, args.rcnn_epochs + 1):
        ...
        print(
            ...
            f"lr={scheduler.get_last_lr()[0]:.2e} t={time.time() - started_at:.0f}s"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("target", choices=["vit", "rcnn", "all"], help="Which training flow to run.")
