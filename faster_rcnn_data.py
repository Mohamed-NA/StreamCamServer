...
def parse_xml_det(xml_path: Path, class_to_idx: dict[str, int]) -> tuple[list[list[float]], list[int]]:
    """Parse Pascal VOC XML into Faster R-CNN box and label tensors."""
...
class FaceMaskDataset(Dataset):
    """
    Dataset for Pascal VOC-style face-mask annotations.

    Images are returned as float32 tensors in [0, 1]. Faster R-CNN handles
...
def collate_fn(batch):
    """Keep images and targets as lists for variable-size detection targets."""
